...
function report(label: string, s: Sim, targetOnInitial: number, tol: number) {
  ...
  const ok = Math.abs(edgeInitial - targetOnInitial) <= tol;
  console.log(
    `[${ok ? "PASS" : "FAIL"}] ${label.padEnd(20)} edge(initial)=${edgeInitial.toFixed(3)}% ` +
      `(target ${targetOnInitial}±${tol})  edge(action)=${edgeAction.toFixed(3)}%`,
  );
  return ok;
}
...
function main() {
  ...
  console.log(`Round games: ${["casino-war"].join(", ")}\nRounds: ${rounds.toLocaleString()}\n`);
  let pass = 0;
  let fail = 0;
